...
internal class SignRepositoryImpl @Inject constructor(
    ...
    override suspend fun signIn(accessToken: String, socialType: String): UserEntity {
        return remoteDataSource.signIn(accessToken, socialType).also {
            saveUserInfo(it, accessToken, socialType)
        }
    }

    override suspend fun signUp(accessToken: String, socialType: String, name: String, isAdNotificationAgree: Boolean): UserEntity {
        return remoteDataSource.signUp(accessToken, socialType, name, isAdNotificationAgree).also {
            saveUserInfo(it, accessToken, socialType)
        }
    }
